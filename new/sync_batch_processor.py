...
class SyncBatchProcessor:
    ...
    def process_sync_operations(self, table_id: str, sync_operations: List[SyncOperation]) -> List[SyncResult]:
        """
        處理同步操作批次
        ...
        try:
            ...
            processed_operations = self._batch_process_fields(sync_operations)
    ...
    def _batch_process_fields(self, sync_operations: List[SyncOperation]) -> List[SyncOperation]:
        """
        批次處理欄位轉換
    ...
    def _execute_update_operations(self, table_id: str, operations: List[SyncOperation]) -> List[SyncResult]:
        ...
        # 更新操作需要逐個處理，因為每個都有不同的 record_id
        for op in operations:
            try:
                ...
                # 單筆更新
                api_start_time = time.time()
                success = self.lark_client.update_record(
                    table_id, op.lark_record_id, op.processed_fields
                )
                self.stats['lark_api_time'] += time.time() - api_start_time
