...
class FieldProcessor:
    ...
    def _load_schema(self):
        ...
        try:
            ...
            self.ticket_fields = schema.get('ticket_fields', [])
    ...
    def _load_config(self):
        ...
        try:
            ...
            if not config_file.exists():
                if self.logger:
                    self.logger.warning(f"配置檔案不存在: {self.config_path}，將使用預設設定")
                return
            ...
            if self.logger:
                self.logger.debug(f"載入配置檔案: {len(self.issue_link_rules)} 個過濾規則")
    ...
    def process_issues(self, raw_issues_dict: Dict[str, Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
        ...
        for issue_key, raw_issue in raw_issues_dict.items():
            try:
                processed_issue = self._process_single_issue(issue_key, raw_issue)
    ...
    def _process_single_issue(self, issue_key: str, raw_issue: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # 確保 Issue Key 欄位存在（從 raw_issue 的 key 或參數 issue_key）
        if 'Issue Key' not in processed_data:
            if issue_key:
                processed_data['Issue Key'] = issue_key
            elif raw_issue.get('key'):
                processed_data['Issue Key'] = raw_issue['key']
    ...
    def _apply_processor(self, processor: str, raw_value: Any, jira_field: str, issue_key: str, config: Dict[str, Any] = None) -> Any:
        ...
        try:
            if processor == "extract_simple":
                ...
                return self._extract_components(raw_value)
            elif processor == "extract_versions":
                return self._extract_versions(raw_value)
    ...
    def _extract_user(self, user_obj: Any) -> Optional[list]:
        ...
        Returns:
            list: Lark Base 人員欄位格式的列表，未找到則返回 None
        """
        if user_obj is None:
            return None
        ...
        if self.user_mapper:
            try:
                ...
                return lark_user_list
            except Exception as e:
                if self.logger:
                    self.logger.error(f"用戶映射失敗: {e}")
                return None
        
        # 如果沒有 UserMapper，返回原始的顯示名稱（向後兼容）
        if isinstance(user_obj, dict):
            # 優先使用 displayName，其次 name
            display_name = user_obj.get('displayName') or user_obj.get('name')
    ...
    def _extract_components(self, components_array: Any) -> Optional[str]:
        """
        提取組件資訊
    ...
    def _extract_versions(self, versions_array: Any) -> Optional[str]:
        """
        提取版本資訊
